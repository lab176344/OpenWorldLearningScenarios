...
class AE(nn.Module):
    def __init__(self,num_labelled,num_unlabelled,image_size,model_depth)->None:
        ...
        self.head1 = nn.Linear(latent_dim,num_labelled)
        self.head2 = nn.Linear(latent_dim,num_unlabelled)
        
    def forward(self,x):
        z = self.encoder(x)
        xhat = self.decoder(z)
        out1 = self.head1(z.view(z.size(0), -1))
        out2 = self.head2(z.view(z.size(0), -1))
 
     
        return out1,out2,z.view(z.size(0), -1),xhat
